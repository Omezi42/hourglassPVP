...
- **`matchmaking live passed` が出ないビルドは上げない。**書き出したpckのまま本物のFirebaseへ
  匿名アカウント2つでつなぎ、ランクマッチの待合室で組んでデッキと種の交換まで通す
  (`tools/tests/matchmaking_live_test.gd`)。差し替え用クライアントのテストでは本物の応答の形の
  食い違いを拾えないため
...
- **仕上げたら `python tools/discord/proofread.py tools/discord/drafts/update-{バージョン}.md`
  でGemini APIに添削させ、指摘を反映する**。ブラウザでGeminiを開かない(ユーザー指示。
  拡張機能経由はトークンを食う)。APIキーは `~/.hourglass_gemini.json` にあり、
  リポジトリへは置かない。APIが混雑(503)なら候補モデルへ順に落ちる
