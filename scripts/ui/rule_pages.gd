...
const CHAPTERS: Array = [
	...
	},
	{
		...
		[
			...
			{
				"heading": "反転権:敵の駒も、いつでも反転できる",
				"body":
				"反転権は敵味方どちらの砂時計も反転させられ、マナは不要です。通常の反転の制限も受けません。相手のまだ若い駒を弱め、跳ね上がった攻撃力を同じ手番の攻撃で仕留めるのが定石です。",
				"stage":
				{
					"kind": STAGE_FLIP,
					"cards": [{"id": "wall", "health": 6, "attack": 1, "caption": "相手の砂時計"}]
				},
			},
		],
	},
	...
	},
	{
		...
		[
			{
				"heading": "6枠の盤面と、24から始まるHP",
				"body": "デッキは30枚(同名2枚まで)。場は各プレイヤー6枠で、相手のHPを先に0にすれば勝ちです。情報帯にはHP・マナ・山札・墓地の枚数が並びます。",
				"stage": {"kind": STAGE_BOARD, "cards": []},
			},
			{
				"heading": "初手は引き直せる(マリガン)",
				"body":
				"配られた初期手札から、好きな枚数を選んで引き直せます。選んだカードは山札へ混ぜてから引くため、同じカードがその場で戻ることはありません。先手・後手が同時に行い、1対局に1度だけです。",
				"stage":
				{
					"kind": STAGE_HAND,
					"cards": [{"id": "grain"}, {"id": "wall"}, {"id": "hammer"}, {"id": "crown"}],
				},
			},
			{
