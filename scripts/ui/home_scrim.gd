...
##
## 帯は3つ。上=アカウント帯、中=タブの中身、下=下部タブ。**上下の帯は中より濃くし、
## 上下で対称にする**——そこに乗るのは真鍮の小さな部品(名札・残高・タブ)で、枠のような
## 大きな面を持たないため、地が明るいと輪郭が背景へ紛れる。中は `HomeFrame` が自分の面を
## 持つぶん薄くてよい。
##
## **画面の端へ向かって濃くする**のが3つに共通の形で、上の帯は上端がいちばん濃く、
## 下の帯は下端がいちばん濃い。こうすると絵の見えている中央がそのまま奥行きになる。
...
## 上の帯と上下対称。**濃さも同じにする**——片方だけ濃いと、画面の重心がそちらへ寄る。
const NAV_STOPS := [
	[0.0, Color(0.09, 0.07, 0.06, 0.72)],
	[1.0, Color(0.05, 0.04, 0.04, 0.86)],
]
...
func _draw() -> void:
	...
	_band(ci, Rect2(0.0, NAV_TOP, size.x, maxf(size.y - NAV_TOP, 0.0)), NAV_STOPS)
