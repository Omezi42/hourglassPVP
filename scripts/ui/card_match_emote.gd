...
func refresh() -> void:
	...
	# **リーサルパズルでは出さない**(GameDesign.md 24章)。相手がいないため送る先が無く、
	# 結果パネルの手前に1つだけ明るいボタンが残ってしまう。
	var puzzling: bool = _screen.puzzle != null and _screen.puzzle.active()
	var can_use: bool = _screen.interactive and not over and not pending_mulligan and not puzzling
