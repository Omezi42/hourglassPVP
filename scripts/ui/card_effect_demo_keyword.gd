...
static func _stage_basic(t: float) -> Dictionary:
	...
	var piece := CardEffectStage.piece(total - step, step, total)
	if step >= total:
...
static func _stage_guard(t: float) -> Dictionary:
	var stage := CardEffectStage.empty_stage()
	var keeper := CardEffectStage.piece(4, 1, 5)
	keeper["guard"] = true
	var mate := CardEffectStage.piece(3, 1, 4)
	var foe := CardEffectStage.piece(3, 3, 6)
	if t < 0.5:
		stage["note"] = "相手は守護を無視して他を攻撃できない"
		stage["beams"] = [
			CardEffectStage.beam(["foe", 0], ["own", 1], CardEffectStage.seg(t, 0.08, 0.4), true)
		]
	else:
		stage["note"] = "攻撃は必ず守護の駒へ向かう"
		stage["beams"] = [
			CardEffectStage.beam(["foe", 0], ["own", 0], CardEffectStage.seg(t, 0.55, 0.85))
		]
		if t >= 0.85:
			keeper["h"] = 1
			keeper["total"] = 2
...
static func _stage_glass(t: float) -> Dictionary:
	...
	var foe := CardEffectStage.piece(3, 2, 5)
	own["glass"] = true
	if t < 0.5:
		stage["note"] = "最初に受けるダメージは硝子が1度だけ無効にする"
		stage["beams"] = [
			CardEffectStage.beam(["foe", 0], ["own", 0], CardEffectStage.seg(t, 0.1, 0.35))
		]
		if t >= 0.35:
			own["glass"] = false
			stage["pops"] = [
				CardEffectStage.pop(
					...
					0,
					"無効",
					CardEffectPreview.BLOCKED_COLOR,
					CardEffectStage.seg(t, 0.35, 0.5)
				)
			]
	else:
		own["glass"] = false
		...
		stage["beams"] = [
			CardEffectStage.beam(["foe", 0], ["own", 0], CardEffectStage.seg(t, 0.55, 0.8))
		]
		if t >= 0.8:
			own["h"] = 2
			own["total"] = 2
			stage["pops"] = [
				CardEffectStage.pop("own", 0, "-2", InkFigure.RED, CardEffectStage.seg(t, 0.8, 1.0))
			]
	stage["own"] = [own]
	stage["foe"] = [foe]
	return stage
...
static func _stage_pierce(t: float) -> Dictionary:
	var stage := CardEffectStage.empty_stage()
	var own := CardEffectStage.piece(3, 5, 8)
	var foe := CardEffectStage.piece(2, 1, 3)
	stage["note"] = "砂時計を攻撃したとき、超過した砂が相手プレイヤーへ抜ける"
	if t < 0.5:
		stage["beams"] = [
			CardEffectStage.beam(["own", 0], ["foe", 0], CardEffectStage.seg(t, 0.12, 0.4))
		]
	if t >= 0.4:
		foe["shatter"] = CardEffectStage.seg(t, 0.4, 0.62)
		own["h"] = 2
		own["total"] = 7
		stage["beams"].append(
			CardEffectStage.beam(["own", 0], ["foe_hp", 0], CardEffectStage.seg(t, 0.5, 0.78))
		)
	if t >= 0.78:
		stage["foe_hp"] = 1.0 - 3.0 / CardEffectPreview.HP_MAX
		stage["pops"] = [
			CardEffectStage.pop("foe_hp", 0, "-3", InkFigure.RED, CardEffectStage.seg(t, 0.78, 1.0))
		]
	stage["own"] = [own]
	stage["foe"] = [foe]
	return stage
...
static func _stage_poison(t: float) -> Dictionary:
	var stage := CardEffectStage.empty_stage()
	var own := CardEffectStage.piece(3, 1, 4)
	var foe := CardEffectStage.piece(6, 0, 6)
	stage["note"] = "1ダメージでも、与えれば相手の砂時計を破壊する"
	# 的が砕けた後も矢印が空を指し続けないよう、当たったところで消す。
	if t < 0.6:
		stage["beams"] = [
			CardEffectStage.beam(["own", 0], ["foe", 0], CardEffectStage.seg(t, 0.15, 0.45))
		]
	if t >= 0.45:
		foe["h"] = 5
		foe["total"] = 5
		foe["shatter"] = CardEffectStage.seg(t, 0.5, 0.75)
	stage["own"] = [own]
	stage["foe"] = [foe]
	return stage
...
static func _stage_lifesteal(t: float) -> Dictionary:
	var stage := CardEffectStage.empty_stage()
	var own := CardEffectStage.piece(4, 3, 7)
	var foe := CardEffectStage.piece(5, 1, 6)
	stage["own_hp"] = 0.6
	...
	if t >= 0.45:
		...
		own["total"] = 6
		stage["own_hp"] = (
			0.6 + (3.0 / CardEffectPreview.HP_MAX) * CardEffectStage.seg(t, 0.45, 0.72)
		)
		stage["pops"] = [
			CardEffectStage.pop(
				"own_hp", 0, "+3", UiPalette.GLOW_AMBER, CardEffectStage.seg(t, 0.45, 0.8)
			)
		]
	stage["own"] = [own]
	stage["foe"] = [foe]
...
static func _stage_double_strike(t: float) -> Dictionary:
	var stage := CardEffectStage.empty_stage()
	var own := CardEffectStage.piece(4, 2, 6)
	var foe := CardEffectStage.piece(6, 0, 6)
	stage["note"] = "同じ砂時計が1ターンに2回攻撃する"
	stage["beams"] = [
		CardEffectStage.beam(["own", 0], ["foe", 0], CardEffectStage.seg(t, 0.1, 0.35))
	]
	if t >= 0.35:
		foe["h"] = 4
		foe["total"] = 4
	if t >= 0.5:
		stage["beams"].append(
			CardEffectStage.beam(["own", 0], ["foe", 0], CardEffectStage.seg(t, 0.5, 0.75))
		)
	if t >= 0.75:
		foe["h"] = 2
		foe["total"] = 2
...
static func _stage_quick(t: float) -> Dictionary:
	...
	var foe := CardEffectStage.piece(5, 0, 5)
	own["fade"] = CardEffectStage.seg(t, 0.0, 0.12)
	...
	if t >= 0.5:
		...
		stage["beams"] = [
			CardEffectStage.beam(["own", 0], ["foe", 0], CardEffectStage.seg(t, 0.55, 0.82))
		]
	if t >= 0.82:
		foe["h"] = 3
		foe["total"] = 3
...
static func _stage_flip(t: float) -> Dictionary:
	var stage := CardEffectStage.empty_stage()
	var flip := CardEffectStage.seg(t, 0.3, 0.65)
	var own := CardEffectStage.piece(1, 4, 5) if flip < 0.5 else CardEffectStage.piece(4, 1, 5)
