...
## 1行だけの知らせ。時刻を本文へ持たせているのは、Discordが同じ投稿者の連続した
## メッセージをまとめてしまい、2件目以降の時刻が画面に出ないため。
## プレイヤーを特定できる情報(表示名・ID)は載せない。
static func build_line() -> String:
	var line := "⌛ 挑戦者が現れました。 [ %s ]" % Time.get_time_string_from_system()
