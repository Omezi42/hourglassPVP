...
## `line_edit` のタップで `window.prompt()` を開き、確定した文字列を反映する。
## `title` はダイアログの見出し。呼ぶのは組み立てた直後の1箇所でよい。
static func wire(line_edit: LineEdit, title: String) -> void:
...
static func _on_gui_input(event: InputEvent, line_edit: LineEdit, title: String) -> void:
	if not ClickArea.is_primary_release(event):
		return
	_prompt(line_edit, title)


static func _prompt(line_edit: LineEdit, title: String) -> void:
	var js := 'window.prompt("%s", "%s")' % [title.json_escape(), line_edit.text.json_escape()]
