...
**失敗したときは連投を抑える印(`_last_sent_at`)を戻す。**戻さないと、届かなかった
まま2分の間隔だけが残り、入り直しても二度と飛ばない。**連投を抑える仕組みが、
届かなかったときにそのまま封じる仕組みとして働いていた。**

結果は `notify_waiting()` の `on_done`(Callable)で返し、`MatchmakingQueue` が
`announce_result` として画面へ流す。**受け口を Callable にしているのは、待っている
