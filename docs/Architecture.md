...
| タブ | クラス | 前身 |
|---|---|---|
| たたかう | `FightTab` | `SoloTab` を作り替え、`BattleTab` のマッチング一式を引き取る |
| そろえる | `DeckTab` | **中身もクラスもそのまま**(デッキ編集 / 図鑑 / ショップ) |
| きろく | `RecordTab` | `BattleTab` から対局の入口を抜いた残り(ミッション / 戦績 / リプレイ) |
| おぼえる | `RulesTab` | **中身もクラスもそのまま** |

- **`DeckTab` と `RulesTab` はクラス名を変えない。**中身が変わらず、`DeckTab` は
  `.tscn` の子として置かれている。**画面に出る名前(そろえる / おぼえる)と内部の
  クラス名が食い違うことは許容する**——`.tscn` のノード名を変えるほうが、参照の
  書き換えという実害のある作業を招く
- **`BattleTab` からマッチングを抜き出すのが作業の本体。**いまこのクラスは
  `MatchmakingQueue` の生成・待機中の巡回ドット・`QueueNotifier` の結果の受け口・
  復帰の判定・戦績とミッションの入口を同時に持っている。**このうち対局に関わるものを
  すべて `FightTab` へ移し、`RecordTab` には記録の3つだけを残す**
- **枠(セクションの見出しつきパネル)は共通の部品へ切り出す。**いま `RulesTab._add_frame()`
  が持っている「`content_panel.tres` のパネル + 見出しのラベル」を `HomeFrame`
  (`scripts/ui/home_frame.gd`)として独立させ、たたかう・きろく・おぼえるの3タブで使う。
  **見出しは真鍮のプレートに載せる**(ラベルだけだと枠の一部に見えず、何の枠なのか読めない)
- **「前回の対局へ戻る」は `FightTab` の最上段の帯として出す**(GameDesign.md 9章)。
  いまは `HomeTile` を縦の列の先頭へ差し込んでいるが、**他の入口と同じ見た目の札では
  急ぐ用件だと分からない**。琥珀の縁を持つ横長の帯にして、左端へ印を打つ
