...
- 詳細は `CardDetailPanel` をそのまま使い、**手札・自分の駒・相手の駒をクリックした時**に、
  指しているカードと反対の端へ出す(ホバーでは出さない)。盤面の左右の余白は190pxしかなく、
  卓へ重ねる以外に置き場が無い。詳細パネル外側のクリックや右クリック/Escキー等で閉じる
...
- **編成中の1行は `CardDeckBand`(`scripts/ui/card_deck_band.gd`)。**高さを46pxに拡大し、
  コストの菱形・名前・2枚積みのバッジ・絵・「−」「+」を1つの `Control` としてゆったり描く。
  **絵は描画先の矩形をボタンとバッジの手前で切って収める**(`_art_rect()`)。
  一覧はスクロール前提とし、太めの帯で視認性と操作性を高める
- **詳細(`CardDetailPanel`)は一覧のカードや編成中の帯をクリックした時のみ画面右上へ固定して出す**
  (`DETAIL_POSITION`)。ホバーによる自動表示は他のカードや編成欄を隠すため廃止。
  固定にしたぶん幅を右カラムぶん(400px)へ絞り、`compact` の中身を横並びから縦積みへ変えてある。
  別のカードをクリックすると切り替わり、詳細パネル外側のクリックや右クリック/Escキー等で閉じる
- **左右のカラム開始位置(y=112)と高さ(584px)を統一する**(`GRID_RECT` / `SIDE_RECT`)。
  以前は絞り込み列のぶん左側だけ下がり、右側上部に46pxの余白ができていたが、モーダル化により
  左右の高さが揃い、一覧グリッドも縦幅が広がってカードを探しやすくなった
- **編成中の欄は上から「デッキ名・枚数 / 一覧スクロール / マナカーブ」とする**。
...
- **連続回数は `MatchState.turn_forfeits` が持つ**。手として送り合うため両者で同じ値になり、
  持ち時間を短くする段(`MatchClock.seconds_after_forfeits()`)と `TURN_FORFEIT_LIMIT` に
  よる敗北判定の両方がこれを見る。**`MatchClock` 側は「回数 → 秒数」の対応だけを持ち、
  状態を持たない**(時計はUIが対局ごとに作り直すため、そこへ回数を置くと復帰・観戦で失われる)
...
- **残り時間が赤くなる境目は固定の秒数ではなく、その手番の持ち時間の半分**とする
  (`PlayerInfoBar.clock_total`)。半減した手番(30秒)は、30秒固定のしきい値だと
...
- **ボタンの見た目はハンバーガー(`UiPaint.Emblem.MENU` / `img_icon_menu_*.tres`)とし、文言を持たない。**`StyleBox` はリソース参照のため `.tscn` のパッチ(値がJSON)では差し替えられず、`HomeScreen._ready()` が `CodedButton.apply_styles()` で指定する。同じ理由で、紋章つきのDiscordボタン(`img_discord_link_*.tres`)も `SettingsPanel` がコードで組み立てて「閉じる」の直前へ挿す
