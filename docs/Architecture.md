...
- **絞り込みと検索は `CardDeckFilter`(`scripts/ui/card_deck_filter.gd`)が1箇所で持つ。**
  コスト・キーワード・名前の3条件を `matches(card)` として合成し、画面側は
  `changed` を受けて並べ直すだけにする。**コストの選択肢はプールに実在するコストから
  作る**(カードが増えて7コストが出た日に、ここを書き足す作業を発生させないため)
- **編成中の1行は `CardDeckBand`(`scripts/ui/card_deck_band.gd`)。**コストの菱形・名前・
  2枚積みのバッジ・絵・「−」「+」を1つの `Control` として描く。**絵は描画先の矩形を
  ボタンとバッジの手前で切って収める**(`_art_rect()`)。帯からはみ出す・バッジに
  重なるといった破綻を、描く側で防ぐ
- **詳細(`CardDetailPanel`)はホバー中だけ浮かせ、出す場所は画面の右上へ固定する**
  (`DETAIL_POSITION`)。以前は指しているカードの隣へ出しており、**次に見たいカードが
  パネルの下に隠れて選べなかった**。固定にしたぶん幅を右カラムぶん(400px)へ絞り、
  `compact` の中身を横並びから縦積みへ変えてある。カーソルが外れたら短い猶予を置いて
  消す(対局画面の詳細と同じ流儀)
- **絞り込みの列(`FILTER_RECT`)は詳細パネルの左端(x=856)までとし、`CardDeckFilter` の
  チップはそこに収まる幅にする。**以前はチップの合計が886pxあり、一覧の幅(816px)に
  対して右端の検索欄がはみ出していた。**チップを足すときは合計幅を数え直すこと**
- **編成中の欄は、絞り込みの列のぶんだけ下げて高さ514pxとする**(`SIDE_RECT`)。
  マナカーブを低くして、同名2枚を軸にした10行前後の構築がスクロールせずに収まる
  高さを保っている(GameDesign.md 9章)
