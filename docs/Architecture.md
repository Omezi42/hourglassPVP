...
デッキで対局へ入れないようにするため)。編集画面は**必ずデッキ一覧から開く**ため、
`open(index)` で何番目のデッキを編集するのかを受け取り(-1 は新規作成)、閉じたら一覧へ戻る。
デッキ名の入力欄は編成中の欄の上端へ置く(共通ヘッダーの右側は保存・プリセット・コードで
埋まっており、入力欄を足すと画面タイトルへ食い込む)。
