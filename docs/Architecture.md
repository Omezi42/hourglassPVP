...
- **ホーム画面のタブ(`DeckTab` / `SoloTab` / `BattleTab` / `RulesTab`)は、上端112pxをアカウント帯のために
...
- **購入は `AccountService.purchase()` の既存の流儀(残高の確認・減算・所有への追加を
  ...
  作らない
- **CPU戦のデッキ生成(GameDesign.md 13章)が所有カードだけから組むかどうかは未確定**
  (`docs/TODO.md` の「カードセット」節を参照)。決まってから `CardDeckSave.random_deck()` を
  改修する
