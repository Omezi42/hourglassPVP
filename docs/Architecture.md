...
- **反転権(GameDesign.md 2章)は「反転する」の直後、`_choose_flip()` の次に検討する**
  (`_choose_flip_right()`)。自分の駒は通常の反転と同じ物差しで得な状態へ戻し、
  相手の駒は`_swap_value()`(既存のSWAP_STATS敵対象の評価をそのまま流用)で見る。
  **相手の駒は「反転させた結果その場で体力が0になる(確実に破壊できる)場合」に限って
  触る。**それ以外(体力を残したまま弱める)は、体力と引き換えに攻撃力を相手へ渡すだけに
  なり、CPUは攻撃を終えたあとにしか反転権を検討しないため、次の相手の手番でその攻撃力を
  そのまま浴びる(`docs/BalanceReport_v5.md` 12章で実測して判明した)。
  **敵の駒に反転トリガー(グロウ/ホイール/ティック/ページ)があれば触らない**——
  誰が反転させても持ち主を利する形で発火するため(GameDesign.md 6章)。同じ理由で、
  破壊する場合も余砂(ON_DEATH)を持つ駒は避ける
  - **希少な資源のため `FLIP_RIGHT_MIN_GAIN` 未満の得なら温存する。**コインと同じく
    使えたかどうかは `use_flip_right()` の戻り値だけで判断する(4.5節・3.1.2節)
  - **この制限を入れると、CPUは反転権をほとんど使わなくなる。**「反転させてから同じ
    手番で仕留める」という2手にまたがる連携を貪欲法は組み立てられないため
    (確実に破壊できる好機自体が少ない)。**砂術と同種のCPUの限界**であり、
    反転権の実測値も過小評価になっている可能性が高い(下記の砂術の注記と同じ扱い)
