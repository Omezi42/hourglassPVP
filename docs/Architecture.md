...
- `MatchReplayController`(`scripts/ui/match_replay_controller.gd`、`_screen`参照を持つRefCounted):リプレイ再生モード(GameDesign.md 12章)の棋譜保持と再生制御を`MatchScreen`から切り出したもの。`MatchResultPresenter`/`MatchTurnResolver`と同じ流儀で、`MatchScreen._ready()`が1個生成し`setup()`で再生コントロール(先頭へ/1手戻る/再生/1手進む/最後へ)と`ReplayTimer`のシグナルを購読する。保持するのは棋譜(`actions`)・現在の手数・再生中フラグ・初期配置の場/控え4本と、手を一気に再現している最中かどうかを示す`catching_up`。入り口は`start_from_firestore(match_id, client)`と`start_from_doc(doc)`の2つで、Firestoreの`matches/{id}`と`LocalReplayService.get_replay()`はいずれもフラットな`Dictionary`(`deck_a`/`deck_b`/`placement_a`/`placement_b`/`actions`)のため後者へ合流する。id配列から初期配置を復元する`ids_to_data()`/`ids_minus()`はstatic関数として持ち、同じ形のドキュメントを読む観戦(`MatchScreen.start_spectate()`)も共用する。**モードフラグ(`_is_replay`)・HUDの出し分け・`GameState`の生成は`MatchScreen`側の責務として残し**、`MatchScreen.begin_replay_mode()`(モード開始)と`MatchScreen.reset_state_for_replay()`(巻き戻しのたびに`GameState`を初期配置から作り直す)経由で呼び出す。`MatchScreen`に残る`start_replay()`/`start_local_replay()`は`Main`向けの薄いラッパー
...
- `ScreenHeader`(`scenes/screen_header.tscn`/`scripts/ui/screen_header.gd`):GameDesign.md 9章の共通レイアウト規約(外周余白24px・ヘッダー高88px・コンテンツ開始y=136)を1箇所で担保する共通コンポーネント。`HBoxContainer`ベースで、左に`BackButton`、中央に`TitleLabel`(`size_flags_horizontal = EXPAND_FILL`で余った幅を吸収し中央揃え)、右に`ActionSlot`(`HBoxContainer`)を持つ。主アクションのボタンは`ScreenHeader`側では持たず、各画面が自分のButtonを`ActionSlot`の子として配置する(ボタンの種類・個数・処理は画面ごとに異なるため)。`back_pressed`シグナルを発行し、各画面はこれを自分の`back_pressed`へ中継する。**各画面がヘッダーの高さ・余白・戻るボタンの位置を個別に決めることは禁止**とし、必ずこのシーンを経由させる
- `HomeScreen`:`TabContainer` を使わず、下部2ボタン(`BottomNav`)で `DeckTab`/`BattleTab` の表示切り替えを行う。将来タブ(ランキング等)を追加する際も既存タブのシーン・スクリプトを変更せず追加できる構造にする
- `HourglassDetailPanel`:砂時計1個分の詳細(イラスト・名前・落下ダメージ・追加効果)を表示する共通コンポーネント。`DeckEditorScreen`・`HourglassListScreen`で共用し、将来対局中の参考表示にも流用できる構造にする
- `HourglassCard`:1駒分のカード表示。`DeckEditorScreen`(ドラッグ元)・`HourglassListScreen`(棚モード)で共用する。**表示サイズは`custom_minimum_size`で自由に決められる**(`Icon`は下端アンカー基準で、通常モードでは名前ラベル分の`ICON_BOTTOM_GAP`を空け、`set_shelf_mode(true)`のときは名前ラベルを隠す分だけカード下端まで広がる)。以前は`Icon`の`offset_bottom`が132px固定で、カードを大きくしてもアイコンの表示サイズが変わらなかった
- `DeckListCard`:デッキ1件分の横長カード。`DeckListScreen`(`Mode.MANAGE`/`Mode.REORDER`)と`BattleDeckPickerScreen`(`Mode.PICKER`)で共用する。**2列グリッドの1列分(約600px)に収まることをレイアウトの制約とする**。内部は`VBox[ HBox[NamePlate, ButtonRow], IconRow ]`の縦2段構成で、砂時計アイコンは5個並べても列幅を超えないサイズに留める。以前は左に名札+ボタンの縦積み・右にアイコン5個を横並びにしていたが、カード1枚で幅796pxを要求し2列に収まらなかった(`BattleDeckPickerScreen`ではアイコンが隣のカードへはみ出していた)
- `DeckListScreen`:`DeckSave` が保持する全デッキを横長カードの2列グリッドとして表示する。各カードから `DeckEditorScreen` への遷移(編集)、並び順の入れ替え、削除を行う。新規デッキ作成ボタンで空デッキを追加し `DeckEditorScreen` へ遷移する
- `DeckEditorScreen`:上段の`Body`(`HBoxContainer`、画面下部を一覧へ譲るため`offset_bottom = -230`)に`MainColumn`(`VBoxContainer`、`EXPAND_FILL`。`NameRow`=デッキ名入力+進捗+保存結果のステータス、`SlotGrid`=`GridContainer` columns=5 の横一列の編成5枠)と`DetailPanel`(302x360の縦長)を並べ、画面下部へ`CardScroll`(全幅、`horizontal_scroll_mode = AUTO`/`vertical = DISABLED`)→`CardRow`(`HBoxContainer`)を置く。砂時計は下の一覧から上の枠へドラッグ&ドロップし、クリックで詳細パネルへ表示する。**一覧を`GridContainer`(5列×2行)にした時期があったが、駒が11種になった時点で3行目が中途半端に切れるため1行の横スクロールへ戻した**(GameDesign.md 9章)。**編成5枠を3+2の2段にし、一覧と詳細パネルを右カラムへ縦積みする左右2カラム構成も試したが、(a)5枠が分断されて「デッキ5枚」として読めない、(b)詳細パネルを横長へ潰すと背景の紋章まわりが窮屈になり砂時計一覧画面と見え方が揃わない、という理由で採らなかった。**
- `HourglassListScreen`:`MatchSetup.all_hourglasses()` で列挙した全種類を表示する(現状は所有/未所有の区別機構が存在しないため全件を通常表示する)
- `BattleDeckPickerScreen`:GameDesign.md 9章の通り、`BattleTab`の「ランダムマッチ」「ルームを作成」「CPU戦」の各ボタンは、押した直後に対戦へ進む前にこの画面を挟む。`Main`が`BattlePurpose`(`RANDOM_MATCH`/`CREATE_ROOM`/`CPU_MATCH`)を覚えておき、この画面の`deck_confirmed`シグナルを受けて元の導線(`BattleTab.begin_random_match()`/`begin_create_room()`/CPU戦の配置生成)を再開する。保存済みデッキを`DeckListCard`の縦スクロール一覧として表示し、タップで選択・確定ボタンで`MatchSetup.player_deck`を確定して`deck_confirmed`を発行する。デッキタブでの「使用中」(`MatchSetup.player_deck`)と一致する保存済みデッキがあれば、開いた時点でそれを初期選択にする。保存済みデッキが0件の場合は一覧の代わりに案内文言を表示し、確定ボタンは無効化する。「戻る」を押すと`BattleTab`側の処理(マッチングキュー参加・ルーム作成等)には一切進まず`HomeScreen`へ戻る
- `MatchScreen` の配置フェーズ(旧`DeckSelectScreen`を廃止し統合。GameDesign.md 9章「配置フェーズは専用画面を設けず、対局画面の中で行う」):自分の手札5枚は既存の`OwnSlotStrip`(`HourglassSlotStrip`)をそのまま流用し、`show_placement_hand(deck, placed_ids, selected_id)`で5枠すべてを選択可能なカードとして表示する(場へ配置済みの駒はこのストリップから消える)。相手の5枚(公開情報)は`OpponentSlotStrip`に同じ表示を`set_placement_interactive(false)`で参照専用として出す。配置先は`GameBoard`の自分の場3マスをそのまま流用し、`GameBoard.show_own_placement(placed)`/`BoardRow.show_placement(placed)`(いずれも`GameState`を介さずHourglassDataの配列を直接表示する、対局フェーズの`show_state()`とは別経路)で表示する。**配置フェーズの3マスは、そのマスの初期状態(`GameState.START_STATES`)のイラストと状態名で表示する**(GameDesign.md 9章)。どこに置くかが初期状態の選択そのものであるため、駒を置く前の空きマスにも状態名を出し、置いた後もそのマスの初期状態のイラストを使う。状態名のラベルは`HourglassSlot`が持つ`PlacementStateLabel`で、配置フェーズ以外では常に非表示にする。相手の場は配置内容が非公開のため常に空表示にする。対局フェーズのHUD(`PlayerStatusBar`・`ActionMenu`・`ReplayControls`等)は配置フェーズ中`visible = false`にし、代わりに`BottomBar/BottomMiddle`配下の`PlacementControls`(効果一覧の横スクロール+「対局開始」ボタン)を表示する。配置フェーズの状態(未配置の手札・場3マスの内容・選択中の駒・確定処理)は`MatchScreen`本体ではなく新規`MatchPlacementController`(`scripts/ui/match_placement_controller.gd`、`extends Node`、`MatchScreen`の子として`_ready()`で1個生成)に切り出し、対局フェーズの責務と分離している。`MatchScreen`は`start_placement_then_match(own_deck, opponent_board, opponent_bench, is_first)`/`start_placement_then_cpu(own_deck, cpu_board, cpu_bench)`/`start_placement_then_online(own_deck, match_id, my_side)`の3つの入り口を持ち、いずれも内部で`MatchPlacementController`へ委譲するだけの薄いラッパーで、既存の`start_match()`/`start_cpu_match()`/`start_online_match()`(引数・処理内容とも無変更)は配置確定後に`MatchPlacementController`からそのまま呼び出される。`MatchScreen.game_board.own_position_pressed`シグナルは配置フェーズ・対局フェーズ共通で受信し、`_on_own_position_pressed()`が`_placement.active`(配置フェーズ中かどうか)で処理を振り分ける
- `MatchSetup`:画面間でデッキ選択結果を受け渡すための静的メンバ(`static var`)を持つクラス。オンライン対戦導入前のローカル動作を成立させるための最小限のつなぎであり、将来の通信層導入時にこの受け渡し方法を見直す
- `DeckSave`:**複数デッキ**(各デッキ=名前+5種類のid配列)を`user://`にJSONの配列として永続化するクラス。並び順は配列の順序で表現する。起動時に読み込み、`DeckListScreen`の表示と`MatchSetup.player_deck`(選択中デッキ)を復元する。旧形式(単一デッキ)の保存データが見つかった場合は、1件のみのデッキ配列として自動移行する
...
- CPUの思考ロジックは `CpuStrategy`(基底クラス、`RefCounted`)として切り出し、`MatchScreen` は具体的な思考の中身を知らずに `choose_action(state, side) -> Dictionary` を呼び出すだけにする。将来より強いCPUを追加する際は `CpuStrategy` を継承した新しいクラスを実装して差し替えるだけで済み、`MatchScreen` 側の変更を必要としない
- 合法手の列挙(反転可能な6駒・移動の3組み合わせ・交代2枠)は `CpuStrategy` の静的関数としてまとめる。これにより、将来の評価関数ベースの実装(各合法手にスコアを付けて最良手を選ぶ等)もこの列挙をそのまま再利用できる
- MVP実装として `RandomCpuStrategy` を用意し、列挙した合法手から一様ランダムに1手を選択する
- CPUの手番になったら、`MatchScreen` が一定時間(0.6秒程度)待ってから `CpuStrategy.choose_action()` を呼び出し、`OnlineMatch.apply()`(既存の静的関数)で着手を反映する。人間側の手と同じ経路で `GameState` へ適用されるため、対局ロジック層に変更は不要
- CPU戦は `MatchScreen.start_placement_then_cpu(own_deck, cpu_board, cpu_bench)` を入り口とする。`Main` がCPU戦開始時にランダムな5枚のCPUデッキと場3個・控え2個への振り分けをあらかじめ生成しておき、プレイヤーが配置フェーズで自分の3個を確定したタイミングで `MatchScreen.start_cpu_match()` を呼び出す
