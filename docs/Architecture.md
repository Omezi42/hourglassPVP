...
| `CardDeckShelf`(`scripts/ui/card_deck_shelf.gd`) | 編成中のデッキを**コスト帯ごとの段**として描く(GameDesign.md 9章)。1つの `Control` が全段を描き、当たり判定を矩形の表として持つ |
...
- **編成中のデッキは `CardDeckShelf`(`scripts/ui/card_deck_shelf.gd`)がコスト帯ごとの段として
  描く**(GameDesign.md 9章)。1枚ずつの `Control` を30個並べるのではなく、
  **1つの `Control` が全段を `_draw()` で描き、当たり判定は矩形の表として持つ**。
  30本ぶんのノードを作ると、枚数を1枚動かすたびに生成と破棄が走るうえ、
  段の高さ・駒の大きさが段ごとに決まるためレイアウトをコンテナへ任せられない
  - **段の高さは `全体 / 段数` と下限(98px)の大きいほう**とし、合計が入りきらなければ
    スクロールする。**縦の上限(`cap_h`)は全段そろえ、横だけ段ごとに詰める**——
    全段そろえる形にすると、種類が集まった1段が全体を道連れにして小さくする
  - **駒の大きさは `min(段の並び幅 / 種類数 - 余白, cap_h)`。**種類が6以下なら縦で頭打ちに
    なるため、普通の構築では結局そろって見える
  - **バッジの半径は駒に比例させる**(`clampf(art_w * 0.21, 8, 14)`)。固定にすると、
    詰まった段でバッジのほうが大きく見える
  - **当たり判定は描画と同じループで `_hit_rects` へ積む**。描く位置と押せる位置を
    別々に計算すると、段の詰まり方が変わったときに必ず食い違う
  - **旧 `CardDeckBand` は共有のデッキ表(`CardDeckSheet`)が使い続ける**ため残す。
    あちらは1枚の画像へ30枚を並べるもので、棚とは目的が違う(横3列の帯のほうが収まる)
- **詳細(`CardDetailPanel`)は `interactive = false` で持ち、一覧のカードや編成中の帯へ
  カーソルを乗せている間だけ出す**(GameDesign.md 9章)。幅は右カラムぶん(400px)で、
  `compact` の中身は縦積み。**出す位置は指しているものと反対のカラム**
  (一覧のカード → `DETAIL_POSITION_RIGHT` / 編成中の帯 → `DETAIL_POSITION_LEFT`)。
  **同じ側へ出すと、パネルが帯を覆った瞬間に `mouse_exited` が飛んで出し消しを繰り返す**
  (帯は詳細パネルの真下にあるため、これは必ず起きる)。外れてから消すまでの猶予は
  対局画面と同じ理由で置く
...
- **編成中の欄は上から「デッキ名・枚数 / コスト帯ごとの棚」とする**。
  **`CardManaCurve` はデッキ編集では使わない**——段の長さがそのままマナカーブになるため
  (GameDesign.md 9章)。クラスは他の用途(共有のデッキ表)が使うので残す
- **30枚に満たない間だけ、棚の下端へ帯を出して残り枚数を示す**(`CardDeckShelf`)。
  そのぶん段に使える高さが減るため、**足りているときと足りないときで駒の大きさが変わる**。
  これは許容する(枚数が変わるたびに揺れるのは編集中だけで、揃ったら止まる)
