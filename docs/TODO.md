...
- [ ] **AA-1 データ層**。`SkillData`(Resource)を新設し、`HourglassData.skill` を追加。
  `EffectType` へ `SWAP_BENCH` / `SWAP_POSITION` を追加する
- [ ] **AA-2 ロジック層**。`GameState` に `{"type": "skill"}` の解決を追加し、
  `EffectResolver.resolve_skill()` を実装する。旧 `move`/`swap_in` は合法手から外す
  (`OnlineMatch.apply()` の分岐は過去のリプレイ再生のため残す)
- [ ] **AA-3 CPU**。`CpuStrategy.legal_actions()` からmove/swap_inを外しスキルを加える。
  `SmartCpuStrategy` にスキルの価値評価を足す
- [ ] **AA-4 UI**。`ActionMenu` を「反転」+スキル名の2ボタン構成へ。交代スキルは控えを選ぶ
  2段フローにする(既存の移動先選択の仕組みを流用)。予約マーク・解決演出・ログの文言を
  スキルに対応させる
- [ ] **AA-5 駒データ**。10枚を `docs/Hourglasses.md` の表のとおりに更新する
- [ ] **AA-6 テスト**と再シミュレーション(2000戦×3シード + メタ進化×3シード)
