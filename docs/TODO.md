...
## 今やること(フェーズ39: 対局中のUXを上げる)

ユーザー指示:「ゲーム中のUXをもっと上げたい」「砂時計を動かして直接殴りにいく感じとかあるといい」。
実際にCPU戦を中盤まで進めて監査したところ、**盤面の操作系(攻撃予測・対象の光らせ方・
ホバー詳細・マリガン・ログ)は既に出来ており、欠けているのは「いま何が起きたか」を伝える層**
だと分かった。GameDesign.md 9章への追記は承認済み。

- [x] **AO-1 手番を画面で示す**。自分の手番になった瞬間に「あなたの番」を一瞬出し
  (`CardMatchTurnFeed`)、手番の側の情報帯の縁を琥珀にする(`PlayerInfoBar.active`)。
  ターン終了ボタンは、まだ出せるカード・攻撃できる駒が残っていれば色を変える
  (判定は `MatchState.has_moves_left()`。ルールの問い合わせなのでロジック層へ置いた)
- [x] **AO-2 攻撃の演出**。`CardView.play_strike()` が「寄る → 溜める → 当てる → 戻る」の
  4段で駒を動かす。**動かすのは絵だけで台座は置いていく**(台座は盤面の設備であり、
  一緒に動くと枠ごと飛んでいくように見える)。絵は `draw_set_transform_matrix()` で
  上端を支点に回し、`Control` 自身の `position` は触らない(触ると当たり判定もずれる)。
  進行は `CardMatchStrike` が持ち、**被ダメージの砂の飛散を当たる瞬間まで持ち越す**。
  本体攻撃・貫通の抜けも同じ動きで表す。同じターンの2回目以降は尺を6割へ詰める
- [x] **AO-3 相手の1手の実況**。`CardMatchTurnFeed` が `CardMatchLog.recorded` を購読し、
  **ログと同じ文言**を起きた駒の近くへ出す。相手の駒は画面の上半分にあるため、
  実況は駒の下(盤面の中央側)へ逃がして絵を隠さない
- [x] **AO-4 ダメージ演出**。HPバーを補間して減らし、被弾したバーを光らせ、増減を
  フローティング数字で出す。**同期は攻撃が当たった瞬間に行う**(戻りきるまで待つと、
  当てた瞬間とHPの減りが対応して見えない)
- [x] **AO-5 操作の細部**。Esc / 右クリックで選択を取り消す。まだ攻撃できる自分の駒の
  台座を控えめに光らせる(`CardView.ready_mark`)

### 付随して直したこと(文字化け)

ユーザー報告:「ルール説明などで使われている三角形がフォントの中にないらしくて
unityroomにアップした後に見ると文字化けしている」。

- [x] **AY-1 フォントに字形が無い記号を使わないようにした**。`▸`(キーワード辞書の
  選択中の印)と `▶`(ルール画面の「手札 → 場」の矢印)を `◆` / `→` へ差し替えた。
  全ソースの文字列をフォントの cmap と突き合わせ、化けるのがこの2つだけであることを
  確認済み。**エディタ実行では別のフォントで代替されるため気づけず、書き出した版でだけ
  化ける**(使える記号の一覧は Architecture.md 4章)
- [x] **AY-2 表示名に使えない文字を弾くようにした**(GameDesign.md 14章・
  Architecture.md 10.5.1節)。`TextGlyphs` がフォント自身へ `has_char()` で問い合わせる。
  **フォールバック用のフォントは同梱しない**(絵文字だけで約10MBあり、pck全体の約4MBと
  釣り合わないため)。入力欄では取り除いて1行示し、相手から届いた表示名は「?」へ置き換える

### 残っている差分

- **連撃の「戻りきる前に2回目へ入る」は未実装**。現状は1回目が台座へ戻ってから2回目が
  始まる(2回目の尺は6割へ詰まる)。演出中は入力とCPUの続きを止めているため、
  重ねるには「戻りの途中から次を受け付ける」形へ変える必要がある

### 進め方の注意

- **攻撃の解決(`MatchState.attack()`)を演出の完了へ依存させない**。依存させると、
  リプレイ・観戦・CPUの連続着手がすべて演出の尺に縛られる。ロジックは即座に済ませ、
  演出は結果を後から見せるだけにする
- `card_match_screen.gd` は993行あり1000行の上限に近い。AO-1/AO-3 の実体は新規
  `CardMatchTurnFeed` へ切り出し、画面側は配線だけに留める
- 過去のセッションで `user://` 配下の実データを誤って削除する事故が発生している。
  検証はヘッドレステストと非ヘッドレスのスクリーンショットのみで行うこと

---


## 完了(対戦相手の募集をDiscordへ通知する)

仕様は GameDesign.md 11章、実装設計は Architecture.md 6.3節(いずれも承認済みで更新済み)。

- [x] **`config/version` を新設した**(`project.godot`)。日付方式 `2026.08.29`
- [x] **`QueueNotifier`**(`scripts/net/queue_notifier.gd`、staticのみ)。1行を投げるだけ。
      `<:sunaeru:...> だれかが対戦相手をさがしてるよ！ [ 19:47:05 ] ・ v2026.08.29`。**時刻を本文へ持たせている**のは、
      Discordが同じ投稿者の連続メッセージをまとめて2件目以降の時刻を隠すため
- [x] **`MatchmakingQueue` へ配線した。**最初の `_try_claim_or_check()` で相手を掴めなかった
      時点が「待機側になった」ことの確定なので、そこで1度だけ呼ぶ。即マッチした場合は
      その手前で return しているため、条件分岐を足さずに仕様を満たしている
- [x] **待機画面には何も出さない**(ユーザー判断)。通知はDiscord側だけで完結させる
- [x] **応答は待たない。**通信の成否でポーリングを遅らせず、失敗しても対局は続く
- [x] 実際にゲーム側のコードから `#通知` へ届くことを確認した

### WebhookのURLの置き場所(重要)

`data/discord_webhook.txt` に置き、**`.gitignore` で管理外にしている**。
**エクスポートはgitではなくファイルシステムを見るため、管理外でもpckには入る。**

**リポジトリへコミットしてはいけない。**GitHubはpublicリポジトリに含まれるDiscordの
Webhookを検出し、Discord側が自動的に無効化する。コミットした時点でこの機能は黙って壊れる。
リポジトリは公開のまま保つ必要がある(BGMの配信にjsDelivrを使うため)ので、非公開にして
回避することもできない。

---

## 次にやること(バージョンが違う相手とマッチングしない)

ユーザー判断:「ver違いはマッチングしなくなるとかもあとで追加しておくべきそう」。
**毎日アプデを運用する以上、これは体裁ではなく正しさの話**になる。ブラウザは古いビルドを
キャッシュしうるため、ルールや値付けを変えた直後は「新しい版」と「古い版」が同じ対局へ
入りうる。v5.0のオンライン対戦は**山札の種を共有して両者が同じ対局を再現する**方式
(Architecture.md 4.0節)なので、カードの総量が1でも違えば**盤面が静かに食い違ったまま
進行する**。落ちないぶん質が悪い。

### まだ決まっていないこと

- 弾く単位。**全ビルドで弾くと、体裁だけ直した回でもマッチングが分断される。**
  「ルールに影響する変更があった回だけ上がる番号」を別に持つ案(例 `rules_version`)の
  ほうが実用的だが、上げ忘れが起きる。要検討

### 着手時の手順

1. **先に GameDesign.md 11章への追記案を出して承認を得る**
2. `MatchmakingQueue` のキュー文書へバージョンを載せ、`_try_claim_or_check()` で
   自分と違う候補を掴まない。ルームマッチも `matches/{id}` の突き合わせで弾く
3. 弾いたときは画面に理由を出す。**黙って待ち続けると「マッチングしない不具合」に見える**
4. 対局画面のどこかにバージョンを出す(不具合報告と突き合わせるため)
...
## 完了(実機での不具合修正: 対局から戻れない・キャンセルが押せない)

ユーザー報告(unityroomの実機):「オンライン対戦はできたが、試合終了後に
『対戦相手が見つかりました!』で止まってしまった」「ランダムマッチに入ったあと、
キャンセルボタンが押せなくてキャンセルができなかった」。仕様は変えていない。

- [x] **AZ-1 対局から戻るとバトルタブが待機状態のまま残っていた**。
  `BattleTab._on_matched()` は待機を止めるだけで `_set_busy(false)` を通らない
  (成立した経路を `_room` の有無で判別してから通知するため、通知の直前に消せない)。
  そのため**ボタンが無効のまま・文言が「対戦相手が見つかりました!」のまま**ホームへ
  戻っていた。`reset_after_match()` を足し、`Main._on_match_back()` から呼ぶようにした。
  あわせて、参照を外すだけで残っていたキュー・ルームのノードを片付ける
  (対戦のたびに子が積み上がっていた)
- [x] **AZ-2 キャンセルボタンが押せなかった**。`CancelButton` が `Margin`
  (全面の `MarginContainer`)より**前**の子だったため、Margin が手前に来てホバーを
  奪っていた。**`MOUSE_FILTER_PASS` はイベントを背面の兄弟へ渡すのではなく親へ渡す**
  ため、押せない状態になる。`CancelButton` を最後の子へ移して解決した
  (`tools/godot_apply_patch.gd` に `move_child` op を追加して適用)

### 検証

**GUIのクリックはヘッドレスでは一切届かない**(`push_input` しても
`gui_get_hovered_control()` は常に none を返す)。非ヘッドレスで `main.tscn` を起動し、
`push_input` でホバーとクリックを流して確認した。キャンセルボタンが
`gui_get_hovered_control()` に出ること・`pressed` が発火すること、成立後の状態から
`reset_battle_tab()` でボタンと文言が戻ることを確認済み。

---

## 完了(フェーズ38: Web版の起動時間の短縮)

ユーザー報告:「unityroomにテスト用でアップしてみたところ起動が遅かった」。
実装設計は Architecture.md 4.1.6節。

**pckが46MBまで膨らんでいた**(前回アップした版は17.9MB)。内訳と対応は次のとおりで、
**14.1MBまで落とした**。wasm(37.7MB・gzipで約9.5MB)は公式テンプレートの固定費のため
手を入れていない(release/nothreadsであることは確認済み)。

- [x] **AV-1 検証用スクリーンショットがpckへ入っていた**。`scratchpad/` と `logs/` へ
  `.gdignore` を置いた(12.0MB)
- [x] **AV-2 参照が0件の画像を削除した**。背景の deck / match、各背景ディレクトリに
  残っていた `button_*.png`(画像ボタン時代の名残)、`assets/背景画像.png`、
  ルートの `tmp_*.png.import`(6.4MB)
- [x] **AV-3 テクスチャを非可逆(WebP)取り込みへ変えた**。砂時計63枚+紋章21枚を
  `lossy_quality=0.85`、背景を0.75(9.1MB → 1.7MB)
- [x] **AV-4 背景を1920x1080を覆う最小サイズへ縮小した**。ホーム 2752x1290 → 2304x1080、
  バトル 2020x1267 → 1920x1204
- [x] **AV-5 検証**。エクスポート済みpckに対する `run_tests.gd` は通る。
  ホーム・砂時計一覧・デッキ編集を実際にレンダリングし、非可逆化による劣化が
  見えないことを確認した

- [x] **AV-6 BGMをpckから外し、実行時に取りに行く形にした**(ユーザー判断「もっと根本的に
  見直してみちゃっていいよ」)。**曲は1秒も削っていない**。Webプリセットの `exclude_filter` で
  pckから外し、`MusicPlayer` が `HTTPRequest` + `AudioStreamOggVorbis.load_from_buffer()` で
  **リポジトリのoggをjsDelivr(CORS対応の無料CDN)経由で**読む。**BGMは自動再生制限で
  最初のクリックまで保留されている**ため、その間にダウンロードが終わり待ち時間にならない。
  あわせて `tools/balance/` の出力(1.1MB)も除外した。**pckは4.1MBになった**
- [x] **AV-7 `tools/export_web.sh` を追加した**。書き出しと、書き出したpckに対する
  `run_tests.gd` を1回で行う

### 判断したこと

**BGMの置き場所は `index.html` の隣ではなくリポジトリ(jsDelivr経由)にした。**
当初は書き出し先の `bgm/` へ素のoggを置く形にしていたが、**unityroomへ上げるのは
`index.pck` だけ**であるとユーザーから示されたため、追加ファイルを配信できない。
曲を短く切る案(pck 8.4MB)とpckへ戻す案(12.9MB)も提示したうえで、ユーザーが
外部ホストを選択した。**リポジトリを公開のまま保つことが条件**になる

### 検証

- 取得と復号は実際にHTTP経由で確認した(title.ogg・2.1MB・100.08秒が
  `load_from_buffer()` で読めること)。デスクトップ側の `res://` 経路も再生を確認した
- エクスポート済みpckに対する `run_tests.gd` は通る

### 残課題

- [ ] **AV-8 フォントが1.66MB**で、残るpck 4.1MBの4割にあたる。デッキ名がユーザーの
  自由入力である以上、サブセット化すると入力できない文字が出るため手を付けていない
- [ ] **AV-9 unityroom上でのBGMの取得は未確認**。jsDelivrがCORSを許していることと、
  実際に取得・復号できることはヘッドレスで確認済みだが、unityroomのページから
  外部への通信が塞がれていないかは実機で見るまで分からない。塞がれていた場合は
  BGMだけが鳴らない(ゲーム自体は動く)

## ロードマップ(フェーズ33〜41・承認済み)

他のカードゲームと比べて足りないものを洗い出し、優先順位をユーザーが承認した。
**UXに関わるものは公開前、カードプールの拡張と収集要素は公開後**という線引きで並べている。
**プレイヤー側の個性(クラス制・ヒーローパワー)とランク・レーティングは採用しない。**

| # | フェーズ | 内容 |
|---|---|---|
| 33 | **マリガン** | 初手の引き直し。ルールの土台のため最初に入れ、バランスの再測定を1度で済ませる |
| 34 | **対局UXの詰め** | 対局中のカード詳細 / ダメージプレビュー / 手札の拡大 / ドラッグでの設置 / 「もう一度」 |
| 35 | **オンボーディング** | プリセットデッキ / 実際に1手指させる誘導対局 / 初回の導線 |
| 36 | **オンラインの完成度** | 切断からの復帰 / デッキコードの共有 |
| 37 | **戦績** | 総勝率・デッキ別・カード別の集計と表示 |
| ─ | **公開** | ここまでが公開前 |
| 38 | **砂を動かす効果群** | 新しい `EffectType` + 余砂持ち + カードプールを21→40種程度へ |
| 39 | **スペル** | 盤面の枠を消費しないカード |
| 40 | **相手の手番への干渉** | 秘策系(条件を満たすと自動で発動する伏せカード) |
| 41 | **収集要素** | 砂金の使い道(ショップ)/ デイリーミッション / スキン・紋章 |

デイリーミッションを41へ置いたのは、砂金の使い道が無いまま配布口だけ増やしても
動機にならないため。**対局中のカード詳細(34の先頭)は GameDesign.md 9章で既に仕様化
されている項目で、v1.0の撤去(AM-5c)で実装だけが落ちたもの**。

---

## 完了(実機での不具合修正: オンライン対戦が始まらない)

ユーザー報告(unityroomのテスト用ページ):「ランダムマッチを押してもマッチングしない」
「キャンセルボタンが押せない」、続けて「マッチングまでは行けたが、そのあとマリガンが
始まらない」。

- [x] **AV-1 対局が始まらなかった原因**。`OnlineSetup.push_setup()` が `Array[String]` を
  受け取るのに、`CardLibrary.ids_from_deck()` が untyped の `Array` を返していた。
  GDScriptは**型が合わない引数で関数を呼ばず、実行時エラーを出して次へ進む**ため、
  デッキが `matches/{id}` へ一度も書き込まれず、**両者が相手のデッキを永久に待っていた**。
  `ids_from_deck()` の戻り値を `Array[String]` にして解決した
- [x] **AV-2 なぜ気づけなかったか**。**オンライン対局の開始経路は、実機まで一度も
  実行されていなかった**(ヘッドレステストは手の送受信だけを見ており、開始のやり取りを
  通していなかった)。`v5_online_tests.gd` に開始のやり取りの検証を追加した。
  あわせて**差し替え用クライアントの上で対局画面2つを実際に起動し**、
  デッキと種の交換 → マリガン → 1ターン目の開始まで、両者の手札が一致することを確認した
- [x] **AV-3 テストがクラッシュで素通りしていた問題**。型で弾かれると、そのテストの
  コルーチンごと打ち切られて残りの検証が走らず、失敗として数えられない。
  **壊れた型そのものを先に検査する**assertを置き、修正前に落ちることを確認した
- [x] **AV-4 キューへ入れなかったときに何も出なかった**。`MatchmakingQueue.join()` が
  書き込みに失敗すると黙って戻っていたため、「押しても何も起きない」ようにしか見えない。
  `failed` シグナルを足して画面へ返すようにした
- [x] **AV-5 キャンセルが効かないように見えた**。後片付けの通信を待ってから画面を
  戻していたため、応答が遅い・返らない場合に固まって見えた。
  **押した瞬間に画面を戻し、後片付けは待たない**形に変えた
- [x] **AV-6 v1.0の配置フェーズ用に残っていた `push_placement()` /
  `wait_for_opponent_placement()` を削除した**(参照0件)

### 教訓

**GDScriptの型付き配列(`Array[String]`)は、untyped の `Array` を渡すと実行時に
関数ごと呼ばれない。**コンパイルは通り、エディタでも該当の経路を通らない限り気づけない。
`Array[T]` を要求する関数へ渡す値は、生成側の戻り値の型まで揃えること。

---

## 完了(フェーズ38: 全画面レイアウトの見直し)

ユーザー指示:「全体的な画面レイアウトをもう一度みんな見直して改善していってみてほしい」。
9画面すべてを実際にレンダリングして確認し、**画面の外へ出ていた3画面**と、
**整列が崩れていた5画面**を直した。仕様(GameDesign.md)は変えていない。

- [x] **AV-1 ホーム「ルール」タブが画面上端で切れていた**。入口3つを縦に積むと
  タブの高さ(448px)を超えるため、「ボタン + その右に説明」の横並び1行に変えた
- [x] **AV-2 ホーム「バトル」タブの最終行が下部タブへ潜り込んでいた**。「戦績」を
  リプレイ・CPU戦と同じ行へ移し、各ボタンとマージンを詰めた
- [x] **AV-3 アカウント画面のパネルが画面の下端を越えていた**。余白とボタンを詰めた
- [x] **AV-4 砂時計一覧**。詳細パネルを右の外周余白へ揃え(右だけ48pxあった)、
  6列を一覧の幅いっぱいへ広げた(余りを列の間隔へ配る)
- [x] **AV-5 デッキ編集**。効果文が「×2」へ接触して切れていたのを省略記号にした
- [x] **AV-6 キーワード辞書**。右カラムの中身(実演の幅520px)がパネル(972px)の
  左へ寄っていたため、パネルの中央へ置いた
- [x] **AV-7 ルール画面**。実演の盤面が下地に直接浮いていたため、他の画面と同じ
  コンテンツパネルへ載せた
- [x] **AV-8 対局画面**。ログ・投了を右の行動列へまとめ、空いた幅で手札を盤面の
  真下(中央)へ戻した。情報帯を両者とも同じ幅にし、卓の縁を明るい琥珀1本から
  真鍮の縁(暗い輪郭+真鍮)へ変えた

### AX群: モーダルとオーバーレイ(ユーザー指示「そこも含めた全てのレイアウトをとっても良くしたい」)

常時表示の画面に続き、重ねて出すもの(モーダル・暗幕・帯)をすべてレンダリングして直した。

- [x] **AX-1 ショップの通知の暗幕に穴が空いていた**。ホーム画面のタブ(高さ560px)の中に
  置かれており、アカウント帯と下部タブにだけ暗幕が掛からず、その2つだけ押せるように
  見えていた。ホーム画面の最後の子へ移した
- [x] **AX-2 デッキ編集のモーダルがカード一覧の背面に描かれていた**。「閉じる」が
  カードに隠れて押せなかった。`Control` は後から足した子ほど手前に描かれるため、
  モーダルの生成を一覧の後ろへ移した
- [x] **AX-3 固定の高さのパネルが空きすぎていた**(墓地・プリセット・デッキコード)。
  中身の高さに合わせて伸び縮みさせ、画面の中央へ置くようにした。墓地は3行を超えたら
  スクロールする
- [x] **AX-4 対局ログに見出しが無かった**。暗幕の上に文だけが浮いていた
- [x] **AX-5 誘導対局の指示帯が手札と重なっていた**(手札を中央へ戻したため)。
  卓の上端へ渡す横長の帯にし、文が2行にならないよう文言を詰めた
- [x] **AX-6 リプレイ再生・観戦から戻れなかった**。結果パネルを出さないモードのため
  出口が存在せず、再読み込みするしかなかった。行動の列の先頭に「戻る」を出す。
  あわせて再生コントロールを行動の列(x=1108)へ揃え、1手ぶんの前後は画面の出口と
  紛れないよう「1手戻/1手進」と書くようにした
- [x] **AX-7 リプレイ一覧のカードから砂時計のアイコンがはみ出していた**。
  カードの高さ(108px)が中身(123px)より低かった
- [x] **AX-8 `card_match_screen.gd` が1000行を超えたため `CardMatchStatus` を切り出した**
  (通信待ちの文言と対象選択の案内。AT-5の一部)

### AW群: 砂時計の絵の縦横比(ユーザー報告)

「砂時計の画像が潰れてたり元のサイズだったりする。とくに、ドラッグしている瞬間だけ
元のサイズになったりするのが嫌だ」。

- [x] **AW-1 絵を正方形の枠へ引き伸ばしていた**。400x513の画像を正方形へ入れていたため
  横に潰れていた。縦横比のまま枠の下端で揃える `CardView._fit_art()` を通すようにした
  (場・手札・反転中・砕け散る演出のすべて)
- [x] **AW-2 状態を切り替えると絵の大きさが跳ねていた**。`state_falling` だけ
  キャンバスが数%大きいため、3状態でいちばん高いものを基準に倍率を共通化した
- [x] **AW-3 ドラッグ中だけ絵が大きくなっていた**。プレビューをカードの枠いっぱい
  (118x158)で作っていたため。札に描かれているのと同じ大きさにした

---

## 完了(フェーズ37: 戦績)

仕様は GameDesign.md 19章、実装設計は Architecture.md 10.7節。

- [x] **AU-1 `MatchStats`**。種別別の通算(対局数・勝ち・手数)、カード別、デッキ別
  (デッキコードで見分ける)をアカウントごとに積み上げる。**リプレイからは集計しない**
- [x] **AU-2 `CardStatsScreen`** とバトルタブの「戦績」ボタン
- [x] **AU-3 AT-5(行数の上限)を解消した**。終局後の後始末を `CardMatchOutcome` へ
  切り出し、`card_match_screen.gd` は997行→981行になった
- [x] **AU-4 砂金の付与が v5.0 の対局画面に配線されていなかったのを直した**。
  結果パネルへ渡す文言が空文字のままで、`CurrencyRules` も `AccountService.grant()` も
  呼ばれていなかった(フェーズ29の AM-5d は完了として記録されていたが、実際には
  対局画面の作り直しで落ちていた)。**「+30 砂金」「10手未満の対局では砂金を獲得
  できません」が出るようになった**
- [x] **AU-5 テスト**。積み上げ・アカウントの分離・同名2枚を1局として数えること

### 残課題

- [ ] **AU-6 実通信での砂金の加算の確認**。判定と表示はローカルで確認済みだが、
  `players/{uid}` への実際の書き込みは未確認(フェーズ27の時点から未確認のまま)

---

## 完了(フェーズ36: オンラインの完成度)

仕様は GameDesign.md 9章・11章、実装設計は Architecture.md 6.2節・10.6節。

- [x] **AT-1 切断からの復帰**(`OnlineResume` + `CardMatchScreen.resume_online_match()`)。
  **局面は保存せず**、`matches/{id}` の「デッキ・種・手の並び」から作り直す
  (リプレイ・観戦と同じ経路)。覚えるのは「どの対局のどちら側か」だけ。
  バトルタブの先頭に「前回の対局へ戻る」を出し、**押した時点で初めて通信する**
  (毎回ホームで通信すると、オフラインでも遊べるという前提を崩すため)
- [x] **AT-2 デッキコード**(`CardDeckCode` + `CardDeckCodePanel`)。
  デッキ編集のヘッダーから、自分のコードの表示・コピーと、受け取ったコードの読み込み。
  **カード一覧の並び順の番号ではなく id と枚数を圧縮する**。番号にすると、カードを
  1枚足した時点で過去のコードがすべて別のデッキになる
- [x] **AT-3 テスト**。デッキコードの往復と壊れたコード、プリセット3つが20枚・同名2枚に
  収まること、復帰の記録の往復(`user://` は控えてから書き、最後に戻す)

### 残課題

- [ ] **AT-4 実通信での復帰の確認**。2クライアントが要るため未実施。
  ロジック(棋譜からの復元・記録の往復)はヘッドレスで押さえてある
- [ ] **AT-5 `card_match_screen.gd` が997行**で、1000行の上限に達しかけている。
  次にこの画面へ足す前に、`MatchReplayController` と同じ要領で切り出す
  (候補: オンラインの開始・復帰・観戦の3経路)

---

## 完了(フェーズ35: オンボーディング)

仕様は GameDesign.md 18章、実装設計は Architecture.md 4.1.5節(いずれも承認済みで更新済み)。

- [x] **AS-1 プリセットデッキ**(`CardPresetDecks`)。「基本 / 速攻 / 重厚」の3つを
  idと枚数の表として持つ。デッキ編集のヘッダーに「プリセット」を足し、選ぶと読み込む
  (保存するまで自分のデッキは変わらない)。**デッキを保存していないプレイヤーは
  「基本」で対局へ入る**(これまでも既定のデッキで入れていたが中身が画面に出ていなかった)
- [x] **AS-2 誘導対局**(`CardMatchTutorial`)。CPU戦へ指示を重ねるだけの作りにし、
  「出す → ターンを終える → 攻撃する → 反転する」の4段階を `MatchState` のシグナルで
  判定して進める。**手は塞がない**。途中で閉じれば通常のCPU戦として続く
- [x] **AS-3 初回の導線**。「ルール」タブの先頭へ「1局遊んで覚える」を置き、
  **まだ遊んでいない間だけ大きく**出す(`UiState.has_done_tutorial()`)

### 判断したこと

- **誘導対局に専用のモードを作らなかった**。作るとルールが2箇所に分かれて食い違う余地が
  生まれるため、`start_cpu_match()` をプリセットで呼んで指示を重ねるだけにした
- **指示の置き場所は手札の右隣**。画面上端へ横長に敷いた版を実際に描いたところ、
  相手のHP・マナ・山札が誘導対局の間ずっと読めなくなった

---

## 完了(フェーズ34: 対局UXの詰め)

仕様は GameDesign.md 9章、実装設計は Architecture.md 4.0節(いずれも承認済みで更新済み)。

- [x] **AR-1 対局中のカード詳細**。**v1.0の撤去(AM-5c)で実装だけが落ちていた**もの。
  `CardDetailPanel` をそのまま使い、**カーソルを乗せている間だけ**、指しているカードと
  反対の端へ出す(盤面の左右の余白は190pxしかなく卓へ重ねるほかない)。語を押すと
  `KeywordPopup` が開くため、対局中でも語の意味を引ける(GameDesign.md 17章)
- [x] **AR-2 ダメージプレビュー**。`MatchState.combat_preview()` が盤面を変えずに結果だけを
  計算し、狙える相手と自分の駒の両方へ「その後の体力 / 破壊」を出す。攻撃側の予測は
  狙える相手が複数いると定まらないため、**最も自分が削られる組**を出す
- [x] **AR-3 手札のホバー拡大**。`CardView.hover_zoom`。位置ではなく scale だけを動かす
  (画面側が毎回 position を置き直すため、位置を動かすと取り合いになる)
- [x] **AR-4 ドラッグでの設置**。`CardView._get_drag_data()` と枠側の `drop_handler`。
  押して枠を選ぶ経路と同じ `_play_selected()` へ合流するため、設置効果の対象選択も働く
- [x] **AR-5 結果パネルの「もう一度」**。当面はCPU戦のみ。オンラインの再戦は相手の同意を
  取り交わす仕組みが要り、片方だけが待ち続ける状態を作りやすいため別途設計する

### 途中で見つけた注意点

**ヘッドレステスト(`run_tests.gd`)は `scripts/ui/` を読まないため、UIのパースエラーを
検出できない。**実際に `card_match_screen.gd` が未定義の識別子を参照したままテストが
通ってしまった。UIを触ったら `--quit-after` での起動スモークまで回すこと。

---

## 完了(フェーズ33: マリガン)

仕様は GameDesign.md 2章・9章、実装設計は Architecture.md 3.1節・4.0節。

- [x] **AQ-1 `MatchState` にマリガンを実装した**。`start_match(..., use_mulligan)` が
  初期手札を配った時点で止まり、`mulligan(side, indices)` は**両者ぶんが揃ってから
  A → B の固定順で適用する**。適用は山札を切り直して乱数を消費するため、届いた順に
  適用すると同じ種から始めた対局が食い違う
- [x] **AQ-2 `MatchAction.mulligan()` と `contains_mulligan()`**。後者は、マリガン導入前に
  保存された棋譜を再生・観戦するときに待たないための判定
- [x] **AQ-3 CPUの選択**(`CardCpuStrategy.choose_mulligan()`)。コスト4以上を戻す
- [x] **AQ-4 マリガン画面(`CardMatchMulligan`)**。選んだカードは沈めて暗くし「戻す」を出す。
  確定後は「相手を待っています」に変わる
- [x] **AQ-5 オンライン対応**。`OnlineSetup` は変えず、**マリガンを手と同じ `actions` の
  1件として送り合う**形にした(送受信・リプレイの経路をそのまま使えるため)
- [x] **AQ-6 テスト4本とバランスの再測定**

### 測定の結果(`docs/BalanceReport_v5.md` 1.4節)

**マリガンは先手勝率をほとんど動かさなかった**(55.5% / 外すと55.9%)。事故を減らす
ぶん両者が等しく得をするためで、「後手を利する」という着手前の見込みは外れた。

- [ ] **AQ-7(残課題)先手勝率が55.5%と目標(45〜55%)をわずかに超えている**。
  マリガンが原因ではなく、**「上書き設置の廃止」以降に生じていた**(それ以前の測定は53.3%)。
  空き枠が無いと出せなくなったぶんテンポの価値が上がったため。**手番の補正は重ねない**
  (GameDesign.md 7章)ので、フェーズ38でカードの値付けを見直す際に併せて測り直す

### 途中で見つけて直した不具合

**コードで生成した全画面オーバーレイが、`set_anchors_preset()` によってサイズ0のまま
固定されていた**。結果パネル・対局ログ・墓地ビューアの暗幕が盤面を覆えておらず、
「終局後に盤面を操作させない」という意図が効いていなかった(Architecture.md 4章に
`SandTransition` の事例として記録済みの落とし穴と同じもの)。

---

## 完了(フェーズ32: キーワード辞書)

ユーザー指示:「キーワード辞書が欲しい」。形は**ゲーム内の一覧画面と、詳細パネル内の
用語ポップの両方**(ユーザー選択)。仕様は GameDesign.md 17章・Architecture.md 4.3節。

- [x] **AP-1 `KeywordEntries` を追加した**。語(常在7種+トリガー3種)→ 表示名・説明・
  実演の `Demo`・分類 の対応表と並び順を1箇所へ持つ。**その語を持つ砂時計は表へ書かず
  `CardLibrary` から実行時に集める**ため、カードを追加しても辞書側の作業は発生しない。
  トリガーと常在は別の enum のため、項目は `{"kind": ..., "value": ...}` の形で持つ
  (値を1つの整数へ混ぜると、衝突していないことを呼び出し側が知っている前提になる)
- [x] **AP-2 `CardEffectPreview.show_demo()` を追加した**。語を直接指定して実演を再生する。
  既存の `show_card()` は `CardData` から台本の並びを組む入口として残した
- [x] **AP-3 `KeywordEntryView` と `KeywordPopup` を追加した**。1語ぶんの表示
  (語 / 説明 / 実演 / その語を持つ砂時計)は `KeywordEntryView` に集約し、
  **辞書画面の右カラムとポップの両方がこれを使う**(2箇所で別々に組むと片方だけ古くなる)
- [x] **AP-4 `CardDetailPanel` の効果欄を語ごとの行にした**。「語のボタン + 説明」の並びに
  なり、押すと `keyword_pressed` が出る。ポップは画面側(`CardListScreen` /
  `CardDeckEditorScreen`)が持つ。あわせて、語のボタンが下端で切れないよう
  イラストと実演を詰めた(実演を大きく見たいときは語を押してポップで見られる)
- [x] **AP-5 `KeywordDictScreen` を追加し、「ルール」タブへ2つ目の入口を置いた**。
  左=語の一覧 / 右=詳細の2カラム。10項目がスクロールなしで収まる高さにしてある

### 実装中に判断したこと

- **選択中の項目は色だけでなく「▸」の印でも示す。**真鍮のボタンの上では文字色の差が
  読み取りにくく、実際にレンダリングして分からなかった
- **実演の幅は 520px で止める。**横いっぱいに伸ばすと、駒が広い枠の中央へ小さく浮いて
  何を見せているのか読めない
- `KeywordPopup` のアンカーは `set_anchors_preset()` ではなく `anchor_right` /
  `anchor_bottom` への直接代入で設定する。フェーズ28の `SandTransition` と同じ罠で、
  生成直後(サイズ0)のノードへ preset を使うと0サイズのまま固定され、
  パネルが画面中央ではなく左上へ寄る(実際にそうなった)

### 残っている作業

- [ ] **対局中の詳細から語を引けるようにする。**GameDesign.md 17章に「対局中に駒を
  クリックして開いた詳細からも同じ操作で引ける」と書いたが、**v5.0 の `CardMatchScreen` は
  まだ `CardDetailPanel` を持っていない**(対局中に駒を押して効果を見る導線そのものが
  未実装)。その導線を作るときに、このパネルと `KeywordPopup` を繋げば満たされる

---

## 今やること(フェーズ31: ルール画面「遊び方」)

ユーザー指示:「下部タブにルールの欄を追加するのはどうだろう。ゲーム開始してすぐにルールが
把握できる感じじゃないとブラウザゲームだし定着してもらえなさそう」「チュートリアルはあっても
いいけど強制じゃなくてあくまでおいておくくらいのイメージ。実際に目にするのはテキストベース
だけど、ゲーム画面も実際にレンダリングされているリッチな紙芝居、みたいなイメージ」。
仕様(GameDesign.md 9章の追記・新章16)と実装設計(Architecture.md 4.2節)は承認済みで更新済み。

- [x] **AO-1 `UiState` を追加する**(`user://ui_state.json`)。ホーム画面を一度でも開いたかを
  記録し、初回だけ「ルール」タブから始めるための判定に使う
- [x] **AO-2 `RulePages` を追加する**。章・見出し・本文・盤面の種類を1箇所の配列へ持つ。
  ページを増やすときはここへ1件足すだけで済む形を保つ
- [x] **AO-3 `RuleStage` を追加する**。`CardView` / `BoardTable` / `PlayerInfoBar` を
  `RulePages` の指定どおりに並べ、`play()` で演出を再生する。**ルール画面専用の描画を
  1つも書かない**ことが要件
- [x] **AO-4 `RuleScreen` を追加する**。目次・本文・`RuleStage`・ページ送り・
  「もう一度見る」。共通の `ScreenHeader` を使う
- [x] **AO-5 `RulesTab` を追加し、ホーム画面へ3つ目のタブとして挿す**。タブとタブボタンは
  `HomeScreen._ready()` がコードで生成し、`scenes/home_screen.tscn` は書き換えない

### 検証

- 非ヘッドレスで `RuleScreen` の全10ページをレンダリングし、砂時計・カード・6枠の盤面・
  情報帯がいずれも対局画面と同じ絵で出ることを確認した。**第5章の相打ちは、当初
  サンド3/2 対 ロック5/3 にしていたため片方が体力0になり、破壊されたはずの駒が
  立ったままになった**。ロックを6/2へ変えて双方が生き残る形にした
- `scenes/main.tscn` を起動してタイトル→ホームへ進み、下部タブが3つになること・
  初回起動なので「ルール」が選ばれていること・タブを切り替えても崩れないこと・
  「遊び方を読む」からルール画面へ入れることをスクリーンショットで確認した
- gdformat / gdlint、`tools/tests/run_tests.gd` はいずれも問題なし

### 進め方の注意

- 過去のセッションで `user://` 配下の実データを誤って削除する事故が発生している。
  **`user://ui_state.json` は新規ファイルだが、検証時に既存の `card_decks.json` 等へ
  一切触れないこと**
- **新しい `class_name` を持つスクリプトを追加した直後は、`godot --headless --path . --import` を
  1度実行する**。これをしないと `--script` 起動時に
...
- [ ] **AM-5 UI**:対局画面の骨格まで実装した。`CardView`(カード1枚)・`PlayerInfoBar`
  ...
  **消える砂と落ちる砂の描き分け**も実装した(`unit_damaged` → 砕けて散る赤の破片 /
  ...
  砂時計一覧(`CardListScreen` + `CardDetailPanel`)と墓地の中身を見るモーダル
...
- [x] **AM-6 イラスト**:20種すべてに割り当てた。**未設定だった10種は既存の絵の色違い**で
  当てている(ユーザー判断「他の砂時計と同じように色だけ変えて当てはめる」)。
  生成は `tools/tint_hourglass_icons.gd` が行い、元id・色相の回転量・彩度を表で持つ。
  カードを増やしたらこの表へ1行足す。**元の絵が鋼や石のようにほぼ無彩色だと色相を
  回しても色が付かない**ため、対象にする彩度の下限を下げて彩度の下駄を履かせている
  (ドリル・ツイン・ロック・スイープ)。固有のイラストを用意したくなったら
  `assets/hourglasses/processed/{id}/` を差し替えるだけでよい

**フェーズ29(v5.0への全面作り直し)は、これで一通り完了。**
残っているのは実通信でのオンライン対戦の確認(2クライアントが要る)のみ。

---

## 今やること(フェーズ30: 対局画面のデザイン刷新)

生成AIへ仕様を渡して作らせた完成画面のモックアップ(5枚)を叩き台に、対局画面を作り直す。
ユーザー判断:「砂時計はそのまま砂時計として表示しても物体として成立しているのに、
わざわざカードの中に閉じ込めるのはもったいない」。**手札はカード / 場は台座の上の砂時計**
という二層構成へ変更する(GameDesign.md 9章・Architecture.md 4.0節は承認済みの内容で更新済み)。

- [x] **AN-1 場のカードを枠なしの砂時計へ作り替えた**。`CardView.Mode.BOARD` は
  「扁平な楕円の台座 + 大きな砂時計 + 足元に攻撃力/体力のバッジ + 名前とキーワード」になり、
  コストは出さない。**台座の輪は絵より後に描く**。守護(太い真鍮の輪)と選択中(水色の輪)は
  駒が立っていても必ず見えなければならず、絵の下に隠すと守護が読めなくなるため
- [x] **AN-2 盤面を1枚の卓上として描いた**。v1.0から残っていた `BoardTable` を12枠の下へ敷き、
  一色べた塗り(輪郭だけの線画に見えていた)を奥から手前へ明るくなる石の面+グレインへ変えた。
  意味を持たない四隅の隅飾りは撤去した(Architecture.md 4章の基準)
- [x] **AN-3 情報帯・行動ボタン・手札の配置を詰めた**。情報帯は角丸+真鍮枠+名札に、
  ターン終了は自分の情報帯の右隣の大きなボタンに、ログと投了は画面下部へ移した。
  **マナのピップが上限10まで並ぶと山札の山に重なる不具合**(以前からあった)も、
  帯の中の横位置を定数へ出して直した。手札は枚数が増えたら重ねて領域内に留める

### 残っている候補

- [x] **AN-4 反転の演出を作り込んだ**(GameDesign.md 9章「反転は他より作り込む」)。
  `MatchState.unit_flipped` を受けて、反転した側の情報帯から対象の駒へ光の筋(`CardFlipBeam`)を
  伸ばし、着弾の閃光の後に駒が持ち上がって裏返り、着地の衝撃波を出す。**自分の駒しか
  反転できないため、筋が上から来るか下から来るかがそのまま「どちらが手を出したか」を示す。**
  **光の筋は独立したオーバーレイのノードにする**。`Control._draw()` は自分の子より背面に
  描かれるため、対局画面の `_draw()` で描くと卓と駒に隠れてほとんど見えない(実際にそうなった)
- [x] **AN-5 全画面のレイアウト監査と微調整**。ユーザー判断「質感とかのアート的なところより
  細かいところのレイアウトやUIの微調整とかを優先」。`main.tscn` を起動して6画面を実際に
  レンダリングし、次を直した。
  - **ボタンの既定の文字色が黒に近く、指定を書き忘れた画面で文字が読めなかった**
    (アカウント画面の「保存」「登録する」「ログイン」、タイトルの「アカウント」)。
    テーマの既定をオフホワイトへ変えて根本から直した
  - **戻るボタンの文字が左の尖った斜辺へ食い込んでいた**(`back_nav` の余白を左右とも指定)
  - **アカウント画面の入力欄の左端が3つとも不揃いだった**(見出しの幅を揃えた)
  - **ホーム画面のヘッダー(アカウント・砂金・設定)の縦位置が揃っていなかった**。
    他画面の共通ヘッダーと同じ y=24〜112 の帯へ合わせた
  - **バトルタブのステータス文言がヘッダーへ重なり、キャンセルボタンが設定ボタンと重なっていた**
  - **砂時計一覧の一覧とデッキ編集のカード行が画面下端の外周余白を割っていた**。
    どちらも下端から逆算した位置・高さに直した
  - **デッキ編集で効果文が伸びて「×2」へ接触していた**(幅を固定して溢れを切る)
  - **砂時計一覧の詳細パネルで効果文が下端で切れていた**(コンテンツ領域の高さいっぱいへ)
  - **リプレイ一覧が通信を待っている間、完全に空だった**(「読み込み中...」を出す)
- [x] **AN-6 画面の質感を底上げした**。卓・情報帯と同じ「多段グラデーション + グレイン +
  落ち込み」を他画面へ広げた。
  - **`ScreenBackdrop` を新設**し、対局・カード一覧・デッキ編集の下地(無地の `ColorRect` 1枚)を
    置き換えた。3画面が個別に下地の色を持っていた状態も解消した
  - **共通ヘッダーにタイトルの暗幕と下端の真鍮線を追加**した。背景イラストが賑やかな画面で
    画面名が読めなかったため。暗幕は中央が濃く左右へ消える形にし、帯に見えないようにした
  - **リプレイ一覧の一覧領域に共通のコンテンツパネルを敷いた**。背景の上にカードと案内文が
    直接浮いていて読みにくかった
  - **HPバーを彫り込まれた溝に見せる**(角丸 + 縦グラデーション + 内側の落ち込み影)。
    山札・墓地・手札の枚数も角丸のプレートにした
  - **マナカーブの棒に角丸・縦グラデーション・上端のハイライトと目盛りの溝を入れ**、
    基線を通した

- [x] **AN-7 カードごとの紋章を追加した**(GameDesign.md 9章)。イラストが全種で共通の1枚の
  色違いであるため、色相だけでは見分けがつかなくなっていた。**カードのモチーフを表す固有の
  アイコン**を21枚ぶん用意し(icooon-mono、商用可・クレジット不要)、`CardData.emblem` として
  持たせて、場は台座の正面のメダル・手札は左下の封蝋・詳細パネルは名前の左に出す。
  **能力の系統を表す共通の紋章を使い回す案は採らなかった**(カードが増えるほど似通うため)。
  **駒の背後に大きな透かしを敷く案も、実際に描いてみて撤回した**(128x168pxでは砂時計の絵が
  ほぼ全面を占め、はみ出した縁だけが見えて散らかる)。取り込み元のSVGは
  `assets/hourglasses/emblems/sources/`(`.gdignore` + `.gitignore`)に置き、
  `tools/build_emblem_icons.gd` が白のシルエットPNGへ焼き直す

### モックアップから採らないもの

生成画像には仕様に無いものが混ざっている。**レイアウトと質感だけを参考にし、語彙は仕様へ戻す**。

- 「ランク戦」タブ(未設計)
- 「挑発 / 突撃 / 再生」(Hearthstoneの語彙。正しくは守護 / 硝子 / 貫通 / 毒砂 / 吸命 / 連撃 / 速落)
- 「価値: 11」(正しくは総量)
- コレクションの所持/未所持の区別(GameDesign.md 9章のとおり、当面は全カードを通常表示)
