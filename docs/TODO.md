...
## 次にやること(駒ごとのバランス調整・いずれも未承認)

ルールと全体量(HP)は確定した。ここからは駒の数値が主題になる。

- [ ] **キングの再調整**。落下ダメージ2→1へ戻すか、自傷2→3で実質2へ戻す(勝率60.2%)
- [ ] **移動(move)の役割の再設計**(1.8%)。現状の用途は「アイの正面をずらす」「ミラーの右隣を
  変える」「落ちきった駒を左マスへ寄せて次の手番に交代で入れ替える」の3つに限られる
- [ ] **交代(swap_in)の改善**(1.8%)。テンポロスは解消されたが選択率は上がっていない。
  左マス限定という制約自体を見直すか
- [ ] **弱い駒の底上げ**。ダッシュ(42.6%)・エコー(42.6%)・ジャッジ(43.7%)・アイ(44.2%)・
  ミラー(44.3%)が下位に固まり、メタ進化でも軒並み消える
- [ ] 判断材料を得るため、メタ進化の母集団と世代数を増やし、シードも5本以上にする
- [ ] `docs/BalanceReport.md` のPlan A / Plan B は**ルール変更前の測定に基づくため破棄**し、
  上記は測り直した数値で判断する
