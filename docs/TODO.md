...
仕様(GameDesign.md 2章・9章)と実装設計(Architecture.md 3.1節・4.0節)は承認済みで更新済み。

- [ ] **AQ-1 `MatchState` にマリガンを実装する**。`start_match(..., use_mulligan)` で
  初期手札を配った時点で止め、`mulligan(side, indices)` が両者ぶん揃ってから
  **A → B の固定順**で適用して `_begin_turn()` へ進む。適用は
  「手札から外す → 同じ枚数を引く → 外したカードを山札へ混ぜて切り直す」の順
- [ ] **AQ-2 `MatchAction` に `mulligan` を足す**。自分の操作・CPU・オンライン・リプレイが
  同じ経路を通るため
- [ ] **AQ-3 CPUの選択**。`CardCpuStrategy.choose_mulligan()`。コストの重いカードを返し、
  軽いカードを残す(1ターン目から出せる手札を作ることが目的)
- [ ] **AQ-4 マリガン画面(`CardMatchMulligan`)**。暗幕+初期手札+確定ボタン。
  確定後は相手待ちを表示する
- [ ] **AQ-5 オンライン対応**。`OnlineSetup` でデッキ・種と一緒に交換した手札の位置を送る
- [ ] **AQ-6 テストとバランスの再測定**。`tools/tests/v5_rules_tests.gd` へ検証を追加し、
  `run_v5_simulation.gd` で5指標を測り直して GameDesign.md 7章の現状値を差し替える

### 進め方の注意

- **マリガンは後手をわずかに利する方向に働く**と見込まれる。先手勝率が目標レンジ
  45〜55% から出た場合は、**手番の補正を重ねずに**(GameDesign.md 7章)コインの側で調整する
- 過去のセッションで `user://` 配下の実データを誤って削除する事故が発生している。
  検証時に既存の `card_decks.json` 等へ一切触れないこと
