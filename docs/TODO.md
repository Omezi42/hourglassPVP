...
- [ ] **AL-1 ハンマー(確定破壊)とスイープ(全体除去)のコストを再測定する**。
  総量を下限まで削っても勝率が下がりきらないため、コストで釣り合わせる必要がある
- [ ] **AL-2 コストが効いていない問題を検討する**。盤面6枠に対しマナ上限10は過剰で、
  中盤以降マナが余る。マナ上限を下げるか盤面を広げるか
...
- [ ] **AM-4 CPU**:合法手の組み合わせが増えるため貪欲法かビームサーチへ作り替える。
  評価関数には**生涯ダメージ `体力×攻撃力 + 体力(体力-1)/2`** を使う
