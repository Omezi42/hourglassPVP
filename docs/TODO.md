...
- [x] **AV-6 BGMをpckから外し、実行時に取りに行く形にした**(ユーザー判断「もっと根本的に
  ...
  pckから外し、`index.html` と同じ階層の `bgm/` へ素のoggを置いて、`MusicPlayer` が
  `HTTPRequest` + `AudioStreamOggVorbis.load_from_buffer()` で読む。**BGMは自動再生制限で
  最初のクリックまで保留されている**ため、その間にダウンロードが終わり待ち時間にならない。
  あわせて `tools/balance/` の出力(1.1MB)も除外した。**pckは4.1MBになった**
- [x] **AV-7 `tools/export_web.sh` を追加した**。書き出しと `bgm/` のコピーを1回で行う。
  手でコピーする形にすると、いつか忘れて無音のまま公開される
...
- [ ] **AV-9 unityroomへ上げるzipに `bgm/` を含めること**。含め忘れるとBGMだけが鳴らない
  (ゲーム自体は動く)。実機での確認は未実施
