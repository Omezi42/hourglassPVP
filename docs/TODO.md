...
## 今やること(フェーズ32: キーワード辞書)

ユーザー指示:「キーワード辞書が欲しい」。形は**ゲーム内の一覧画面と、詳細パネル内の
用語ポップの両方**(ユーザー選択)。仕様(GameDesign.md 17章)と実装設計
(Architecture.md 4.3節)は承認済みで更新済み。

- [ ] **AP-1 `KeywordEntries` を追加する**。語(常在7種+トリガー3種)→ 表示名・説明・
  実演の `Demo`・分類 の対応表と並び順を1箇所へ持つ。その語を持つ砂時計は表へ書かず
  `CardLibrary` から実行時に集める(カード追加時に辞書を書き換えないため)
- [ ] **AP-2 `CardEffectPreview.show_demo()` を追加する**。語を直接指定して実演を
  再生できるようにする。既存の `show_card()` は `CardData` から台本の並びを組む入口として残す
- [ ] **AP-3 `KeywordPopup` を追加する**。1語ぶんの説明・実演・該当砂時計を出すモーダル
- [ ] **AP-4 `CardDetailPanel` の効果欄を、語ごとに押せる形へ分ける**。現在は `Label` 1つへ
  全文を流し込んでいる。押すと `KeywordPopup` が開く。対局中の詳細も同じパネルを使うため
  1箇所の変更で両方へ効く
- [ ] **AP-5 `KeywordDictScreen` を追加し、ホーム画面の「ルール」タブへ入口を置く**。
  左=語の一覧 / 右=詳細の2カラム。共通の `ScreenHeader` を使う

### 進め方の注意

- AP-1 → AP-2 → AP-3 の順に進める(下が上に依存する)。AP-4 と AP-5 は AP-3 の完了後
...
- **フェーズ31(ルール画面)がホーム画面の「ルール」タブを新設する。**AP-5 はそのタブへ
  入口を置くため、タブが未実装の場合はこのフェーズで最小限のタブを作るか、
  フェーズ31の完了を待つかを着手時に判断する
