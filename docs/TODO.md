...
- [ ] **AM-6 イラスト**:未設定の10種(グラス・ドリル・ヴァンプ・ロック・ツイン・ランス・
  ...
  絵を流用している。**生成プロンプトをカードごとに提示する必要はない**
  (`docs/AssetPromptTemplate.md` のテンプレートからユーザーが直接作る)。
  こちら側の作業は、`assets/hourglasses/incoming/{id}.png` へ置かれた画像の取り込み
  (3分割・正規化・`processed/` への配置)と `.tres` へのイラスト参照の反映
