...
- [ ] **AV-6 BGM 3曲で8.96MB**を占めており、残るpckの7割にあたる。この環境には
  ffmpeg/oggenc が無いため再エンコードできず、**曲を短く切り出す**(`scratchpad/ogg_trim.py`
  がページ境界で無劣化に切る)以外の手段がない。プレイヤーが聴く長さが変わるため、
  ユーザーの判断を待っている
- [ ] **AV-7 フォントが1.66MB**。デッキ名がユーザーの自由入力である以上、
