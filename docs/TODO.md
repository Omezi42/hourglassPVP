...
## 次にやること(バージョンの明示とマッチングの制限)

ユーザー判断:「verも表示しておいたほうがよさそう」「**ver違いはマッチングしなくなる**とかも
あとで追加しておくべきそう」。**毎日アプデを運用する以上、これは体裁の話ではなく正しさの話**に
なる。ブラウザは古いビルドをキャッシュしうるため、ルールや値付けを変えた直後は
「新しい版」と「古い版」が同じ対局へ入りうる。v5.0のオンライン対戦は
**山札の種を共有して両者が同じ対局を再現する**方式(Architecture.md 4.0節)なので、
カードの総量が1でも違えば**盤面が静かに食い違ったまま進行する**。落ちないぶん質が悪い。

### まだ決まっていないこと

- **`project.godot` に `config/version` が無い。** まずここを決める。日付方式
  (`2026.08.30`、同日に複数回出すなら `-2`)が毎日アプデとは噛み合う
...
1. **先に GameDesign.md 11章への追記案を出して承認を得る**(バージョンは仕様に一度も
   登場していない)
...
- Discordのインキュー通知にはバージョンを載せる形にした
  (`⌛ 挑戦者が現れました。 [ 17:51:23 ] ・ v2026.08.29`)。ゲーム側に定数ができたら
  そこから読む
