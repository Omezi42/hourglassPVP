...
- [ ] **AO-1 手番を画面で示す**。自分の手番になった瞬間に「あなたの番」を一瞬出し、相手の
  手番の間は相手の情報帯を光らせて自分の情報帯を落とす。**ターン終了ボタンは、まだ出せる
  カード・攻撃できる駒・払えるマナが残っているときに色を変える**(9章に元からあるが未実装)
- [ ] **AO-2 攻撃の演出**。`CardView.play_strike()` が「寄る → 溜める → 当てる → 戻る」の
  4段で駒を動かす。**駒は上端を支点に振れる**。相打ちは防御側の砂の飛散と揺れで示す。
  本体攻撃・貫通・連撃もこの動きで表す。同じターンの2回目以降は尺を6割へ詰める
- [ ] **AO-3 相手の1手の実況**。対象へスポットライトを当て、ログと同じ文言を近くへ浮かせ、
  間を置いてから次の手へ進む。CPUがまとめて指すと何をされたか追えないため
- [ ] **AO-4 ダメージ演出**。HPバーを補間して減らし、被弾したバーを光らせ、減った量を
  フローティング数字で出す。**`hp_changed` はどこにも繋がっていなかった**(9章の未実装分)
- [ ] **AO-5 操作の細部**。Esc / 右クリックで選択を取り消す。まだ攻撃できる自分の駒に
  控えめな印を出す
