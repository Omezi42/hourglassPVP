...
- [ ] **AL-6 ハンマーとスイープの効果を弱める(要承認)**。総量が下限(2)に達しており、
  コストを上げても勝率が下がらない(スイープはコスト7で63.5%、9でも60.4%)。
  提案は「ハンマー:1体を破壊 → **1体に4ダメージ**(コスト4・総量3、実測48.9%)」
  「スイープ:全体の砂を2粒削る → **1粒削る**(コスト6・総量4、実測51.1%)」。
  **この2枚が勝率を吸っているため残り18種が一律50%を下回っており、
  確定するまで全体の再調整ができない**
- [ ] **AL-7 逆転率が43.4%と高い**(目標25〜35%)。手を入れるならルール側になるため、
  実際に遊んでからの判断とする

### AL-4: UI仕様の改訂(**承認待ち**)

`docs/GameDesign_v5_ui_draft.md` に GameDesign.md 9章の差し替え案を作成し、
実際のフォント・カードデータで描いた画面モックを提示済み。判断が必要な点は
「体力=カード左上・攻撃力=カード左下(一般のDCGと逆)」「キーワードは紋章でなく語」
「消える砂と落ちる砂を演出で分ける」の3点。承認後に9章へ反映し AM-5 へ入る。
