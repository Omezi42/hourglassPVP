...
- [ ] **AO-1 `UiState` を追加する**(`user://ui_state.json`)。ホーム画面を一度でも開いたかを
  記録し、初回だけ「ルール」タブから始めるための判定に使う
- [ ] **AO-2 `RulePages` を追加する**。章・見出し・本文・盤面の種類を1箇所の配列へ持つ。
  ページを増やすときはここへ1件足すだけで済む形を保つ
- [ ] **AO-3 `RuleStage` を追加する**。`CardView` / `BoardTable` / `PlayerInfoBar` を
  `RulePages` の指定どおりに並べ、`play()` で演出を再生する。**ルール画面専用の描画を
  1つも書かない**ことが要件
- [ ] **AO-4 `RuleScreen` を追加する**。目次・本文・`RuleStage`・ページ送り・
  「もう一度見る」。共通の `ScreenHeader` を使う
- [ ] **AO-5 `RulesTab` を追加し、ホーム画面へ3つ目のタブとして挿す**。タブとタブボタンは
