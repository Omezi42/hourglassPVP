...
- **「メール/パスワード」プロバイダの有効化は不要だった**。AK-1の確認時に
  `accounts:signUp` が `WEAK_PASSWORD` を返した(無効なら `OPERATION_NOT_ALLOWED` になる)
  ため、既に有効になっていることが分かっている
- **メール列挙保護を無効化する必要もない**。`accounts:signUp` によるリンクは
  有効なままでも通る
