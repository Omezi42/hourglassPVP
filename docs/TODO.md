...
- [ ] **AL-7 先手勝率が60.4%。コインの導入を提案する(要承認)**。
  ユーザー指示により「手札は変えず、コインの導入を検討」した結果、
  **後手にコインを渡し、あわせて「先手の1ターン目はドローなし」を廃止する**構成で
  **5指標すべてが初めて目標圏に入った**(先手53.3%・決着23.4手・本体決着98.5%・
  ユニット攻撃52.2%・逆転率34.6%)。ハースストーンと同じ形(先手3枚 / 後手4枚+コイン)になる。
  **コイン単独では約22ポイントを動かす非常に強い補正**で、1ターン目ドローなし(同じく約22pt)と
  併用すると先手38.2%まで行き過ぎる。実装(`MatchState.use_coin()` / `COIN_ENABLED`)と
  テストは入れてあるが、**既定は無効**にしてある(GameDesign.md 2章の変更にあたるため)
