...
中身は両者の山札のid 30枚ずつで、**並びがそのまま引く順**。`MatchState.keep_deck_order`(既定は偽)を真にして
山札を切らずに始め、プレイヤーを先手、CPUを `BEGINNER` に固定する。台本をコードへ書かないのは、
カードの追加・調整で段階が成立しなくなったときに `.tres` だけを直せば済むようにするため。
成立は `tools/tests/` の自動テストが確かめる(台本の山札で、段階の求める操作を指すだけの単純な手を打ち続けたとき、
決まった手番数のうちに全段階が巡り、プレイヤーが勝つ)。

**段階の定義(文・完了の合図・光らせる対象・数字の光)は `CardMatchTutorial` から切り出したデータ側に持つ**。
8段へ増えると本体の処理と混ざって読みにくくなるため。操作を求める輪郭と、読んでいる数字の光(`CardMatchGeometry` から
位置を取る)は色を分ける。**1度だけの補足**(砂の落ちきりで割れた / 相手が守護を出した)は段階とは別の待ち行列1本で持ち、
帯が段階の説明を出していない時に出す。出したかどうかはその対局の中だけで覚え、保存しない。
