...
- **キューに残った切断済みプレイヤーを掴まない**。ブラウザを閉じたプレイヤーのキュー
  ...
  - **ポーリングで自分のキュードキュメントが404なら書き直す。**裏のタブではメインループが止まり
    ...
    誰からも見えないまま待ち続ける
  - **心拍とCPU戦の印は、自分の文書が在るときだけ書き換える**(`exists: true` を前提条件にした
    `commit()`。`_touch_waiting_doc()`)。掃除された後に一部のフィールドだけで作り直すと
    `match_id` を持たない文書になり、`match_id == ""` の検索に掛からないうえ、自分のポーリングも
    「在る」と見て書き直さない
  - 検証は2段で持つ。`matchmaking_flow_tests.gd` が差し替え用クライアントの上で複数人の
    `join()` を実際に回し(後から入る・同時に入る・3人目・掃除の後・CPU戦から戻る・キャンセル)、
    `matchmaking_live_test.gd` が本物のFirebaseで2人を組ませてデッキと種の交換まで通す
    (`tools/export_web.sh` が書き出したpckに対して回す)
...
| `RoomCodeTiles`(`scripts/ui/room_code_tiles.gd`) | 4桁のコードを1桁ずつの升に描く。`editable` のときは透明な `LineEdit` を重ねて入力を受け、数字以外を落とす(モバイルの `MobileTextInput` もこの `LineEdit` へ掛ける) |
...
**相手の席は `AccountService.fetch_profile()` で埋める**(`room_ready` の `opponent_uid`)。
読み終える前に部屋を出た場合に古い応答で席を上書きしないよう、応答が届いた時点で
同じ部屋にいるかを確かめてから描く。
